// The modal/pop-up to be shown as you click on the outcome card

import { X, Trash2 } from "lucide-react";
import { useEffect, useState } from "react";

import { useUpdateOutcome, useDeleteOutcome } from "@/hooks/useOutcomes";
...
export default function OutcomeModal({ outcome, onClose }: OutcomeModalProps) {
    ...
    const [value, setValue] = useState(String(outcome.value));
    const [dueDate, setDueDate] = useState(outcome.due_date);
    const [paid, setPaid] = useState(outcome.paid);
    const [observation, setObservation] = useState(outcome.observation ?? '');

    // Show delete confirmation buttons
    const [isConfirmationButtonVisible, setIsConfirmationButtonVisible] = useState(false);

    const { mutate, isPending, error } = useUpdateOutcome();
            ...
            >
                ...
                <div className="mt-3 flex items-center justify-between border-t border-border-main pt-3">
                    ...
                    <div className="inline-flex">
                        <button
                            onClick={() => deleteOutcome({ id: outcome.id }, { onSuccess: onClose })}
                            disabled={isDeleting}
